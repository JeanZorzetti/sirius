...
// ── read_file ─────────────────────────────────────────────────────────────────
server.tool('read_file', 'Read any file from the Sirius CRM project. Path can be relative to project root or absolute.', { path: z.string() }, async ({ path: p }) => {
    const content = await fs.readFile(resolveSafe(p), 'utf-8');
    return { content: [{ type: 'text', text: content }] };
});
// ── write_file ────────────────────────────────────────────────────────────────
server.tool('write_file', 'Write or overwrite a file. Creates parent directories automatically.', { path: z.string(), content: z.string() }, async ({ path: p, content }) => {
...
// ── edit_file ─────────────────────────────────────────────────────────────────
server.tool('edit_file', 'Replace a string inside a file. Fails if old_string is not found.', {
    path: z.string(),
    old_string: z.string(),
    new_string: z.string(),
...
// ── delete_file ───────────────────────────────────────────────────────────────
server.tool('delete_file', 'Delete a file from the project.', { path: z.string() }, async ({ path: p }) => {
    await fs.unlink(resolveSafe(p));
    return { content: [{ type: 'text', text: `Deleted: ${p}` }] };
});
// ── list_files ────────────────────────────────────────────────────────────────
server.tool('list_files', 'List files in a project directory. Set recursive=true to walk subdirectories (skips node_modules, .next, .git).', {
    path: z.string().optional(),
    recursive: z.boolean().optional(),
}, async ({ path: p, recursive }) => {
    const full = resolveSafe(p ?? '');
    if (recursive) {
        const { stdout } = await execAsync(`find "${full}" -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" -not -path "*/dist/*"`);
...
// ── search_files ──────────────────────────────────────────────────────────────
server.tool('search_files', 'Grep for a pattern across project source files. Returns up to 100 matches.', {
    pattern: z.string(),
...
// ── run_command ───────────────────────────────────────────────────────────────
server.tool('run_command', 'Run a shell command in the project root (or a specified subdirectory). DATABASE_URL is pre-set.', {
    command: z.string(),
...
// ── db_query ──────────────────────────────────────────────────────────────────
server.tool('db_query', 'Execute a raw SQL query against the Sirius CRM production database (postgres). Returns JSON rows.', {
    sql: z.string(),
...
// ── get_schema ────────────────────────────────────────────────────────────────
server.tool('get_schema', 'Read the Prisma schema (all database models and their fields).', {}, async () => {
    const content = await fs.readFile(path.join(PROJECT_ROOT, 'prisma/schema.prisma'), 'utf-8');
    return { content: [{ type: 'text', text: content }] };
});
// ── get_project_info ──────────────────────────────────────────────────────────
server.tool('get_project_info', 'Get a quick overview of the Sirius CRM project: stack, DB, paths, and coding conventions from CLAUDE.md.', {}, async () => {
