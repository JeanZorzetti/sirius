...
### Removed
- **47 arquivos sem nenhum importador** (commit `c1c5d3d`, US3 de `002-remove-dead-code`): `lib/mobile/*` (6), `components/mobile/*` (6), `app/[locale]/dashboard/tasks/actions.ts`, órfãos de tasks/chat/dashboard/marketing/captação (`components/tasks/*`, `components/chat/resizable-drawer.tsx`, `chat-drawer.tsx`, `calculadora-roi-with-lead-capture.tsx`, `lead-capture-modal.tsx`, `marketing/hero-scroll.tsx`, entre outros), libs órfãs (`lib/integrations/google-calendar-automations.ts`, `whatsapp-automations.ts`, `lib/nlp/entity-disambiguation.ts`, `lib/email-i18n.ts`), `components/microsoft-clarity.tsx` (Clarity tracking, resíduo morto — CSP e menção no blog ficaram para o T076), e 7 órfãos de 2ª ordem descobertos ao reescanear após a leva inicial (`hooks/useNotifications.ts`, `components/ui/drawer.tsx`, `lib/mobile/ocr.ts`, `components/ui/container-scroll-animation.tsx`, `lib/whatsapp-sync.ts`, `app/[locale]/(admin)/admin/funnel/actions.ts`, `components/plan/usage-limits-banner.tsx`). Nenhum tinha consumidor no repositório; nenhum aponta para um commit de deleção específico (código morto de nascença, não órfão de refactor). Zerou `ARQUIVOS SEM IMPORTADOR` do scanner e ligou o gate `--check` da CI (a partir daqui, vermelho no job `dead-code` é regressão de verdade).
...
- **31 rotas de API sem chamador e libs AGI que caíam junto** (commit `cbc3f32`, US5): 9 rotas AGI/NLP vítimas diretas de `2d29773` (`/api/agi/query`, `enrich`, `diagnose`, `recommend`, `learning-path`, `explain-relationship`, `test`, `diagnostic`, `/api/nlp/extract`) + as libs `lib/agi/graph-skills.ts` e `lib/nlp/auto-citation.ts` que só elas alcançavam (confirmado que `lib/nlp/graph-rag.ts`/`graph-queries.ts`, vivos, não dependem delas); mais 15 rotas administrativas/integrações órfãs (`whatsapp/settings`, `whatsapp/diagnostic`, `admin/billing/charge-overdue`, `admin/organizations` + `[id]/tier`, `admin/support/stats`, `scraping/jobs/cleanup`, `debug/pusher-test`, `agenda/calendar-status`, `notifications` + `/stream` + `/[id]` + `/mark-all-read`, `pusher/auth`, `leads/capture-calculator`); `app/api/billing/upgrade/` (fluxo de cobrança Mercado Pago substituído por Stripe, confirmado sem chamador e sem plano MP configurado em nenhum `.env*`); e o diretório `lib/mercado-pago/` inteiro (`checkout.ts` + `products.ts` órfão de 2ª ordem + teste). `app/api/mercadopago/checkout/` (redirect de 7 linhas usado pelo app mobile antigo) foi preservada e entrou na allowlist. Rotas sem chamador caíram de 34 para 3, todas justificadas.
- **6 rotas de migração one-shot** (commit `cbc3f32`, US5) convertidas em scripts executáveis (`scripts/migrate-deals-pipeline.ts`, `fix-unread.ts`, `fix-waba-id.ts`, `add-closings-permission.ts`, `reset-wa-db.ts` — este com guarda de confirmação por ser destrutivo) e apagadas de `app/api/admin/`; `sync-contacts` foi apagada sem virar script (já era um stub `410 Gone` sem lógica, a sincronização real dependia do gateway whatsmeow descontinuado).

### Changed
- **Formatadores e limites de plano consolidados numa implementação por conceito** (commit `0a68d8c`, US7 de `002-remove-dead-code`): as 17 implementações reais de `formatCurrency`/`formatDate`/`formatPhone`/`timeAgo` espalhadas pelo código (25 candidatas, 8 excluídas por serem contrato de API externa ou não formatadores de exibição) unificadas em `lib/format.ts`, com 22 testes de caso de borda; `lib/feature-gates.ts` e `lib/plan-limits.ts` fundidos em `lib/entitlements.ts` (17 importadores repontados); `lib/hooks/use-entitlements.ts` movido para `hooks/use-entitlements.ts`. Duas mudanças de comportamento visível, deliberadas: telefones com DDI em `components/chat/conversation-item.tsx` passam a mostrar "+55"; `timeAgo` em `components/ia/ia-knowledge.tsx` ganha granularidade fina (minutos/horas) em vez do bucket "hoje"/"ontem".
