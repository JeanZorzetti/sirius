...
- [X] T064 [US7] Antes de escrever `lib/format.ts`: catalogar num rascunho as 25 implementações (10× `formatCurrency`, 10× `formatDate`, 3× `formatPhone`, 2× `timeAgo`) e **as divergências de comportamento entre elas** — valor nulo, data inválida, telefone com e sem DDI. É o catálogo que define o alvo, não a primeira implementação encontrada. **Resultado, diferente do esperado**: grep real achou 7 `formatCurrency` (não 10), 5 `formatDate` reais + 4 excluídos por serem contrato de API/parsing anti-fuso (não 10), 3 `formatPhone` (bate exato) e 2 `timeAgo` (um deles chamado `relativeTime`, não `timeAgo`) — mesma divergência de contagem já vista em US3/US5, fonte de verdade é o grep, não a lista congelada. Excluídos da consolidação, com motivo: `lib/google-search-console.ts` (formatDate = contrato ISO com API externa), `lib/api-helpers.ts` (formatDate = serialização ISO para resposta de API, não exibição), `lib/seo-forecasting.ts` (parsing manual de string pra evitar bug de fuso horário que um `new Date()` genérico reintroduziria), `components/dashboard/deal-mobile-card.tsx` (não é formatador de data, é contagem regressiva de prazo).
- [X] T065 [US7] Criar `lib/format.ts` conforme [data-model.md](./data-model.md#libformatts-novo-us7). **Divergência deliberada do data-model.md, registrada aqui**: `formatDate` ficou sobre `Intl`/`toLocaleDateString` (a técnica que 100% das implementações antigas já usava), não sobre `date-fns` — trocar de técnica arriscava mudar a saída renderizada sem necessidade (FR-020 pede saída idêntica, date-fns não estava em uso em nenhum dos 5 call sites reais). `timeAgo` também não foi para `Intl.RelativeTimeFormat('pt-BR')` como o data-model sugeria — esse formatter produz "há 2 dias", texto visivelmente diferente do "2d atrás" que já está em produção; manter o formato custom evita quebrar FR-020/T072 em todas as telas que usam `timeAgo`. `formatCurrency` ganhou um 2º parâmetro opcional (`Intl.NumberFormatOptions`) para acomodar os call sites que usavam 0 casas decimais sem alterar o default (2 casas) dos demais — nenhum call site muda de comportamento. `formatPhone` uniu as 3 implementações (a de `conversation-item.tsx` era a única sem prefixo "+55" nos números com DDI e a única com o branch de 10 dígitos); a decisão, registrada aqui, foi manter o prefixo "+55" (maioria 2 de 3, incluindo o painel de contato) e absorver o branch de 10 dígitos como adição — `conversation-item.tsx` é o único call site com mudança visível (números de 12/13 dígitos passam a mostrar "+55").
- [X] T066 [US7] Teste em `lib/__tests__/format.test.ts` (22 testes) cobrindo os casos de borda catalogados no T064: valor nulo/undefined/NaN em `formatCurrency`, data inválida/nula em `formatDate`, DDI com/sem 55 e 10/11/12/13 dígitos em `formatPhone`, os 5 degraus de `timeAgo` (agora/min/h/d/fallback). 22/22 passando isolado.
- [X] T067 [US7] Migrados os 25 call sites reais (7+5+3+2, ver T064) para `lib/format.ts`; definições locais apagadas. Confirmado: `grep` por `function formatCurrency|const formatCurrency`, `function formatPhone|const formatPhone` e `function timeAgo|const timeAgo` fora de `lib/format.ts` vem vazio; `function formatDate|const formatDate` só resta nos 4 excluídos do T064 (documentados) + `lib/format.ts`. `components/tasks/analytics/completion-trend-chart.tsx` tinha um wrapper redundante em cima de `formatDateBR` (já compartilhado, US7 não mexe nele) — removido, chamando `formatDateBR` direto.
- [X] T068 [US7] Fundido `lib/feature-gates.ts` (623 linhas) e `lib/plan-limits.ts` (209) em `lib/entitlements.ts` — todas as funções, classes de erro e a interface `OrganizationEntitlements` movidas para o arquivo único, sem mudança de assinatura.
- [X] T069 [US7] Repontados os importadores para `lib/entitlements.ts`: 13 de `feature-gates.ts` + 4 de `plan-limits.ts` (não 3 — mesma divergência de contagem do T064; `app/api/v1/deals/route.ts` também importava). Mocks de teste ajustados (`__tests__/api/v1/deals.test.ts` mockava `@/lib/plan-limits`, agora mocka `@/lib/entitlements`). `lib/__tests__/feature-gates.test.ts` teve só o import corrigido (26 testes, sem mudança de conteúdo). Os dois arquivos absorvidos apagados (Acceptance US7-2).
- [X] ~~T070 [P] [US7] Renomear `hooks/useNotifications.ts`~~ **Obsoleta, já registrada no handoff antes desta sessão**: o arquivo foi apagado na cascata de 2ª ordem da US3, junto com seu único importador (`notification-center.tsx`). Nada a renomear.
- [X] T071 [P] [US7] `lib/hooks/use-entitlements.ts` e `lib/hooks/__tests__/use-entitlements.test.tsx` movidos para `hooks/` via `git mv` (preserva histórico); `lib/hooks/` apagado (FR-022). 5 importadores repontados para `@/hooks/use-entitlements`. **Achado na migração**: o próprio `use-entitlements.ts` importava `OrganizationEntitlements` via caminho relativo (`../feature-gates`, depois `../entitlements`) — relativo a `lib/hooks/`, quebrava depois do `git mv` para `hooks/` (subiria um nível a menos). Trocado por `@/lib/entitlements` (absoluto), imune à posição do arquivo.
- [X] T072 [US7] **Verificação estrutural, não visual em navegador** — mesma indisponibilidade de Postgres remoto (`31.97.23.166:5434`) catalogada desde T044/US4. Evidência substituta: os 22 testes do T066 fixam o valor exato renderizado (com espaço NBSP do `Intl.NumberFormat`, formato `dd/mm/aaaa`, etc.) para os casos de borda de cada implementação absorvida; cada call site migrado no T067 recebeu o `style`/`opts` que reproduz exatamente a saída da sua implementação local antiga (registrado por arquivo no T065). Únicas duas mudanças visíveis conscientes, ambas documentadas no T064/T065: `conversation-item.tsx` (telefone com DDI ganha "+55") e `ia-knowledge.tsx`/`task-activity-feed.tsx` (timeAgo unificado na granularidade fina — minutos/horas — em vez do bucket "hoje"/"ontem"). **Lacuna registrada para a próxima sessão com banco acessível**: confirmar visualmente nas telas de deals/contatos/dashboard.
- [X] T073 [US7] Tríade + build. `npx tsc --noEmit` limpo. `npx vitest run`: 350 passed/1 skipped (0 falhas — a flakiness de `deal-isolation.test.ts` catalogada desde T019 não se reproduziu nesta rodada), 12,49s locais — não comparável 1:1 com os 676 testes/18,34s oficiais do T013 (contagem caiu por todas as deleções acumuladas de US1-US6, não por esta fase; SC-007 fica para a CI confirmar). `node scripts/audit-dead-code.js --check`: sai `0`, `ARQUIVOS SEM IMPORTADOR: 0`, `ROTAS SEM CHAMADOR: 3/212` (as 3 da allowlist, inalterado — US7 não mexe em rotas). `npx next build`: 591/591 páginas estáticas geradas, parou no mesmo `EINVAL` Windows/OneDrive de sempre (catalogado desde T019) e nos mesmos erros de Postgres remoto em páginas de blog (catalogado desde T019/T044) — nenhum erro novo, CI (Linux) decide.
...
- [X] T074 Provar o gate (SC-006): num PR **descartável**, apagar uma página que seja a única consumidora de uma rota de API e abrir o PR. Esperado: a CI **reprova**, nomeando a rota órfã. É a reprodução exata de `2d29773`. Enquanto este teste não reprovar, a FR-023 não está entregue — mesmo com o job existindo e verde. Descartar o PR depois. **Feito**: PR [#9](https://github.com/JeanZorzetti/sirius/pull/9) (branch `002-t074-prove-gate`) apagou `app/[locale]/dashboard/settings/integrations/logs/page.tsx`, única consumidora de `/api/integrations/logs` (confirmado por grep: só o próprio `route.ts` e a page referenciam essa rota). Reproduzido local (`--check` saiu `1`, nomeando a rota) **e** na CI (job `Dead Code Scan`, run `32758778659`, `conclusion: failure`, log com o mesmo `--check: 1 item(ns) fora da allowlist: - /api/integrations/logs`). PR fechado sem merge, branch remota e local apagadas.
- [X] T075 Medir o resultado final contra `baseline.md`: arquivos sem importador `0`, rotas sem chamador `≤3` todas com motivo escrito, `≥24.000` linhas removidas, 12 dependências a menos, CI verde em `main` com testes executando (SC-001, SC-002, SC-003). **Resultado**: `node scripts/audit-dead-code.js` — `ARQUIVOS SEM IMPORTADOR: 0` (baseline: 59/7.368 linhas), `ROTAS DE API SEM CHAMADOR: 3/212` (baseline: 32/247), as 3 na allowlist com motivo. `git diff --shortstat 7309f03 HEAD` (commit anterior ao início da spec 002, excluindo `package-lock.json` e `specs/`): **239 arquivos, 1.587 inserções, 34.451 deleções — líquido ~32.864 linhas removidas**, acima do alvo de 24.000. Dependências: 11 removidas (não 12, divergência do T017 — `openai` continua em uso real por `lib/rag/embeddings.ts`), confirmado por grep no `package.json` atual. CI em `main`: run mais recente (`7ba137e`) `conclusion: success`. `npx tsc --noEmit` limpo; `npx vitest run` 349 passed/1 failed/1 skipped (mesma flakiness pré-existente de `deal-isolation.test.ts`, catalogada desde T019, não regressão).
- [X] T076 [P] Varrer `docs/` por qualquer outro documento que descreva como em produção algo removido nas fases 5–8, e corrigir ou apagar (SC-008). **Feito**: apagados `docs/QUICK_START_GENUI.md` (guia de acesso a `/dashboard/agi-genui` "em produção") e `scripts/COMO_TESTAR.md` + `scripts/test-genui-endpoint.js` (guia e script de teste de `/api/agi/chat-with-ui` contra `siriuscrm.com.br`, ambos inteiramente sobre o subsistema apagado no US6). `roadmaps/roadmap_en.md` corrigido em 3 pontos: bloco de inventário "Generative UI (17 components)" trocado por nota de remoção; 6 linhas de rotas admin já mortas desde `2d29773` removidas da tabela (`ab-testing`, `auto-citation`, `generative-ui`, `graph-rag`, `spin-chat`, `cache-stats`); item 14 da lista de gaps riscado com nota. `docs/AUDITORIA_OVER_ENGINEERING_2026-08-22.md` e `docs/ROADMAP_SECURITY_STABILITY.md` deixados como estão — descrevem o subsistema como candidato à remoção (não como em produção) e a segunda já está marcada `[x]` concluída; `docs/ROADMAP_REFATORACAO_2026.md` menciona "generative-ui" só como nome de chunk numa nota histórica de bundle-splitting, não como funcionalidade ativa — deixado.
- [X] T077 [P] Fechar o `CHANGELOG.md` com as remoções grandes de US3, US5 e US7 que ainda não estejam registradas, cada uma com motivo e commit de origem (FR-019). **Feito**: 3 entradas novas em `[Unreleased]`/`Removed` (US3 — 47 arquivos sem importador, commit `c1c5d3d`; US5 — 31 rotas + libs AGI + `lib/mercado-pago/`, commit `cbc3f32`; as 6 migrações one-shot viraram scripts) e 1 em `Changed` (US7 — formatadores e `lib/entitlements.ts` consolidados, commit `0a68d8c`).
- [X] T078 Escrever `handoff.md` co-localizado com o resultado das 72h de Sentry de cada deploy (SC-004) e o que ficou de fora: o service account `sirius-crm-483316-a2e815438069.json` na raiz **continua lá** — é achado de segurança fora do escopo desta feature, pede `/security-review` e rotação em passagem própria. **Feito** — ver handoff.md.
