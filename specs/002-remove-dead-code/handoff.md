...
**Feature**: `002-remove-dead-code` | **Escrito em**: 2026-08-24 (atualizado após US5, mergeada) | **Para**: próxima sessão que continuar via `/speckit-implement`

## Onde parou

**US0 (Fase 1+2), US1 (Fase 3), US2 (Fase 4), US6 (Fase 8), US3 (Fase 5), US4 (Fase 6) e US5 (Fase 7) concluídas e mergeadas em `main`** — commits `521cc65` (PR #1), `75fcf11` (PR #2, US1), `6543fc8` (PR #3, US2), `f70086a` (PR #4, US6), `c1c5d3d` (PR #5, US3, squash), `eb181f4` (PR #6, US4) e `cbc3f32` (PR #7, US5, squash). T001–T053 e T055–T063 todos `[X]` em [tasks.md](./tasks.md). Branch `002-us5-rotas-orfas-agi-migracoes` já apagada (local e remota).

**T054 (monitorar Sentry 72h pós-deploy) fica pendente** — a merge foi confirmada nesta sessão (`gh pr view 7` → `MERGED`, `cbc3f327f20008738bbcc0301a3bba5911b7161b`), mas não houve verificação do Sentry ainda. Quem retomar a partir de **2026-08-27** (72h depois de 2026-08-24T12:50:58Z) pode considerar essa janela fechada e registrar o resultado (ou a ausência de acesso ao Sentry) aqui.

Próximo passo: **US7** (T064 em diante, Fase 9) — já estava desbloqueada desde que US3+US6 fecharam. Depois dela só falta o Polish (Fase 10, T074+).
...
1. `git status` na `main` — confirmar limpa (deve mostrar só os untracked de sempre, ver seção acima) e `git log -1` mostrando `cbc3f32` (US5) como topo, ou algo mais recente.
2. Próxima fase é **US7** (Fase 9, T064 em diante) — já desbloqueada, sem dependência pendente. Criar branch a partir de `main` atualizada (ex.: `002-us7-consolidacao-conceitos-duplicados`), executar T064-T073, tríade local, PR, CI, **perguntar antes de mergear** — mesmo fluxo de sempre. Ver a nota "US7" no topo deste handoff sobre a T070 estar parcialmente obsoleta (`hooks/useNotifications.ts` já não existe, apagado na cascata da US3).
3. Depois da US7: só falta o Polish (Fase 10, T074+) — `tasks.md` tem a lista completa, incluindo o T077 (fechar `CHANGELOG.md` com as remoções grandes de US3/US5/US7) e o T076 (docs órfãs deixadas de propósito pela US6, mais o achado desta sessão de entradas stale em `i18n/routing.ts` apontando pra páginas do Generative UI já apagadas — `/admin/auto-citation`, `/admin/ab-testing`, `/admin/generative-ui`, `/admin/graph-rag`, `/admin/spin-chat` — fora do escopo da US5, registrar ali).
...
5. **`node scripts/audit-dead-code.js --check` está verde desde o merge da US5** (0 arquivos, 3 rotas, todas na allowlist, confirmado na CI do PR #7). Se aparecer vermelho a partir de agora, é regressão de verdade.
6. **T054 pendente**: verificar Sentry por 404/500 novos entre 2026-08-24 e 2026-08-27, registrar o resultado aqui quando alguém tiver acesso.
