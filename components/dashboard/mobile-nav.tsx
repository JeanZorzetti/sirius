...
function MobileNavLink({ item, pathname, onClose }: { item: NavItem; pathname: string; onClose: () => void }) {
        ...
        <div className="flex h-full flex-col relative z-10">
          ...
          <ScrollArea className="flex-1">
            <nav className="grid items-start px-3 pt-4 gap-0.5">
              ...
              {crmItems.map((item) => (
                <MobileNavLink key={item.href} item={item} pathname={pathname} onClose={close} />
              ))}
