...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-600">
      {children}
    </span>
  )
}
    ...
    <div className="w-64 h-screen border-r border-border bg-sidebar/80 backdrop-blur-xl relative z-50">
      ...
      <div className="flex h-full flex-col relative z-10">
        ...
        <ScrollArea className="flex-1">
          <nav className="grid items-start px-3 pt-4 gap-0.5">
            {/* WhatsApp Founder Button */}
            <a
              href="https://wa.me/5562983443919?text=Olá! Gostaria de falar com o fundador."
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-300 mb-2 text-white bg-green-500 hover:bg-green-600 shadow-sm hover:shadow-green-500/30"
            >
        ...
        {/* Footer - Account */}
        <div className="border-t border-border px-3 py-3 grid gap-0.5">
