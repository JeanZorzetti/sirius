    ...
    <div className="overflow-hidden rounded-xl border border-border/50 bg-card">
      ...
      <div className="grid grid-cols-7 border-b border-border/50 bg-muted/20">
        {WEEKDAYS.map((day) => (
          <div
            key={day}
            className="px-1 sm:px-2 py-2 text-center text-[8px] sm:text-[10px] font-semibold uppercase tracking-wide text-muted-foreground"
          >
            {day}
          </div>
        ))}
        ...
        {grid.map((cell, i) => {
          ...
          return (
            <div
              ...
              className={cn(
                'group relative min-h-[80px] sm:min-h-[100px] cursor-pointer border-b border-r border-border/30 p-1 sm:p-1.5 transition-colors hover:bg-muted/20',
              ...
              <div className="flex flex-col gap-0.5">
                {visible.slice(0, 2).map((task) => (
                  <button
                    ...
                    className={cn(
                      'group/task flex items-center gap-1 truncate rounded px-1 sm:px-1.5 py-0.5 text-left text-[8px] sm:text-[10px] hover:bg-muted/40',
