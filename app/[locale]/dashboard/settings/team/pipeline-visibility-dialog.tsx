...
interface PipelineVisibilityDialogProps {
    ...
    currentCanViewDealClosings: boolean
}
...
export function PipelineVisibilityDialog({
    ...
    const [canViewDealClosings, setCanViewDealClosings] = useState(currentCanViewDealClosings)
    ...
    const handleSave = () => {
        startTransition(async () => {
            try {
                await updateUserPipelineVisibility(userId, restricted, selectedIds, canViewDealClosings)
    ...
    const handleOpenChange = (val: boolean) => {
        if (val) {
            ...
            setCanViewDealClosings(currentCanViewDealClosings)
        }
        setOpen(val)
    }
            ...
            <DialogContent className="bg-zinc-950 border-zinc-800 text-zinc-100 sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-zinc-400">
                        Configure o que <strong className="text-zinc-300">{userName}</strong> pode ver.
                    </DialogDescription>
                </DialogHeader>

                <div className="py-4 space-y-4">
                    {/* ── Seção: Pipelines visíveis ── */}
                    <div className="space-y-2">
                        <p className="text-xs font-medium text-zinc-400 uppercase tracking-wide px-0.5">Pipelines visíveis</p>

                        <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                            !restricted ? 'border-green-500/40 bg-green-500/5' : 'border-zinc-800 hover:border-zinc-700'
                        }`}>
                            <Checkbox
                                checked={!restricted}
                        ...
                        <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                            restricted && selectedIds.length === 0 ? 'border-red-500/40 bg-red-500/5' : 'border-zinc-800 hover:border-zinc-700'
                        }`}>
                            <Checkbox
                    ...
                    {/* ── Seção: Permissões dentro dos cards ── */}
                    <div className="space-y-2">
                        <p className="text-xs font-medium text-zinc-400 uppercase tracking-wide px-0.5">Dentro dos cards</p>

                        <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                            !canViewDealClosings ? 'border-red-500/40 bg-red-500/5' : 'border-zinc-800 hover:border-zinc-700'
                        }`}>
                            <Checkbox
                                checked={!canViewDealClosings}
                                onCheckedChange={(checked) => setCanViewDealClosings(!checked)}
                                className="data-[state=checked]:bg-red-600 data-[state=checked]:border-red-600"
                            />
                            <div className="flex-1">
                                <div className="flex items-center gap-1.5">
                                    <DollarSign className="h-3.5 w-3.5 text-zinc-400" />
                                    <p className="text-sm font-medium text-zinc-200">Bloquear aba Fechamentos</p>
                                </div>
                                <p className="text-xs text-zinc-500 mt-0.5">Usuário não verá a aba de fechamentos nos cards do pipeline</p>
                            </div>
                        </label>
                    </div>
                </div>
